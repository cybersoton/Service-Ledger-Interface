...
function readAfterCheck(req, res, flag){
  if(!flag){    
    res.writeHead(404);
    res.end('The requested metrics does not exist!\n');
  } else {
      ...
      path: config.keyValueStore
        ...
        function(response_payloads) {
            for (let i = 0; i < 1; i++) {                
                ...
                availability = jsonLogs.Availability;
                responseTime = jsonLogs.ResponseTime;
                cpuPower = jsonLogs.CPUPower;
                diskSpace = jsonLogs.DiskSpace;
                memorySize = jsonLogs.MemorySize;
                bandwidth = jsonLogs.Bandwidth;
                throughput = jsonLogs.Throughput;
                connections = jsonLogs.Connections;
                elasticity = jsonLogs.Elasticity;                
...
module.exports.store = function(req,res){
   ...
   */
  if (requestorID == undefined || token == undefined || timeStamp == undefined || availability == undefined || responseTime == undefined
    || CPUPower == undefined || diskSpace == undefined || memorySize == undefined || bandwidth == undefined || throughput == undefined
    || connections == undefined || elasticity == undefined) {
    res.writeHead(400);
    ...
    hfc.newDefaultKeyValueStore({
      ...
      function(response) {
        if (response.status === 'SUCCESS') {                
          logger.info('Successfully stored the SLA Metrics!');

          eventhub.registerChaincodeEvent(config.chaincodeID, 'storeMetrics', function(result){
            logger.info('======Store Metrics chaincode event received!======');
            var id = result.payload;
            logger.info("Store Metrics Event:" + id.toString('utf8'));                                      
            var json = JSON.stringify({ 
