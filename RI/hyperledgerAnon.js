...
var logger = log4js.getLogger('ANON');
...
function readAfterCheck(req, res, flag){
  if(!flag){    
    res.writeHead(404);
    res.end('The anonymisation data has not been found!\n');
  } else {
      ...
      path: config.keyValueStore
        ...
        function(response_payloads) {            
            for (let i = 0; i < 1; i++) {
                ...
                data_provider = jsonLogs.DataProvider;
                data_consumer = jsonLogs.DataConsumer;
...
module.exports.register = function(req,res){
   ...
   */
  if (requestorID == undefined || token == undefined || timeStamp == undefined || data_consumer == undefined || data_provider == undefined
    || dataID == undefined) {
    res.writeHead(400);
    ...
    hfc.newDefaultKeyValueStore({
      ...
      function(response) {        
        if (response.status === 'SUCCESS') {                
          var json = JSON.stringify({ 
            "message": "Anonymisation metrics have been successfully registered!"
          });                        
          res.writeHead(200, {"Content-Type": "application/json"});
          res.end(json + '\n');

          eventhub.registerChaincodeEvent("anon", 'registerAnon', function(result){
            logger.info('======Register Anon chaincode event received!======');
            var id = result.payload.toString('utf8');
            logger.info("Register Anon:" + id);            
          });                              
        }
      }
    ).catch(
...
module.exports.read = function(req,res){
  ...
  if (requestorID == undefined || token == undefined || data_consumer == undefined) {
    res.writeHead(400);
    res.end('Invalid request, required parameter(s) missing!\n');
  } else if(!validate(token)) {    
    res.writeHead(401);
    ...
    keyFoundTest(data_consumer, req, res, "read");
  }
}
